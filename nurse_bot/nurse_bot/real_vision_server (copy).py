...
class RealVisionServer(ImgNode):  # ImgNode를 상속받아서 기능 확장
    def __init__(self):
        ...
        tool_model = os.path.join(pkg_path, "resource", "tool_pose.pt")
        hand_model = os.path.join(pkg_path, "resource", "hand_pose.pt") # 파일명 확인!
    ...
    def detect_callback(self, request, response):
        ...
        # 5. 추론
        results = model(self.color_frame, verbose=False)
        ...
        for r in results:
            for box in r.boxes:
                ...
                is_match = (target == 'hand') or (target.lower() in cls_name.lower())
                
                if is_match and float(box.conf[0]) > 0.5:
